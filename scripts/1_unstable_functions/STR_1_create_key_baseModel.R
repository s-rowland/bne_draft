# File: STR_1_create_key_baseModel.R
# Author: Sebastian T. Rowland <sr3463@cumc.columbia.edu>
# Date: 12/22/21

#' \code{create_key_baseModel} creates a reference grid with a user-specified density
#' 
...
create_baseModelKey <- function(ref.df, refName, baseModel.df, baseModelName, 
                                timeScale) {
  ...
  # 3b. identify the indicies of the relevant JS centroids
  # this step should take ~20 seconds
  tictoc::tic()
  baseModelIndices <- unlist(nngeo::st_nn(refLoc.sf, baseModelLoc.sf, k = 1))
  tictoc::toc()
  
  # 3c. create table linking the aqs and the js locations 
  # we can subsequently use this key for filtering 
  ref_baseModel_key <- baseModelLoc.df[baseModelIndices, ] %>%
    cbind(refLoc.df, .) %>%
    tibble::as_tibble() %>%
