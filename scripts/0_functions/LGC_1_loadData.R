...
#'    \item{\code{obs_pm25} a double giving the PM2.5 observation recorded at the EPA AQS monitor on the specified date.}
#' }
...
loadData <- function(path, dataset) {
  ...
  if (dataset == "AQS_annual"){
    ...
    # "JS_annual", "CACES_annual", 'MERRA_annual', 'CAMS_annual'
       ...
       capture_date <- stringr::str_split(stringr::str_split(path, "js/")[[1]][2], ".rds")[[1]][1]
