...
key.refGridConus.cm <- read_fst(here::here('inputs', 'pm25', 'keys', 
                                           'key_nn_refGridConus_cmDaily.fst')) %>% 
  arrange(ref_lat) %>% 
  arrange(ref_lon)
...
# 1.h create empty training list
training.list <- c()
...
cm <- cm %>% 
  mutate(ddate_diff = as.POSIXct(Date) - parse_date_time(paste(yyyy, '-01-01'), 'ymd')) %>% 
...
maxDoY <- 365 
if (yyyy %in% c(2004, 2008, 20012, 2016)) {maxDoY <- 366}
...
training.full <- foreach(
  ...
  #### ------------------- ####
    ...
    preds.day <- preds.day %>% 
      mutate(day_of_year = 100 * dayOfYear/maxDoY,
