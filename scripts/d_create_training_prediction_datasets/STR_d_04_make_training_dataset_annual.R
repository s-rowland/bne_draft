...
datasetCodes <- c('AV', 'GS', 'CMAQOUTS_annual',  'CACES', 'MERRA_annual') # 'JS_annual',
modelNames <- c("av", "gs", "cmaq_outs",  "caces", 'merra') # 'js',
...
for (i in 1:length(YYYYlist)){
  ...
  # 2C.a. save results
  refGrid.yyyy %>% 
    dplyr::select(lat, lon, year, obs_pm2_5, av_pred, gs_pred, cmaq_outs_pred, js_pred, caces_pred, merra_pred, ref_id ) %>% 
    readr::write_csv(here::here('BNE_inputs', 'training_datasets', 'individual_annual', 
                                paste0('training_avgscmjsccme_', YYYYlist[i], '_all.csv')))
}
