...
predCount = readtable(append('PredCount_', YYYY, '_AVGSCMJSCC_1percent.csv'));

fid = fopen(append('Predictions_', YYYY, '_AVGSCMJSCC_1percent.csv'),'r'); % Creates id for reading in data line by line. A lot of the code below is 
...
% save results 
%writematrix(X, append('X_', fold, '.csv'))
%writematrix(y_mean, append('y_mean_', fold, '.csv'))
