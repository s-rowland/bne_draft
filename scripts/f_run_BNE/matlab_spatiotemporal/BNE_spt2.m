function [W,w0,SigW,Z,piZ, Zt] = BNE_spt2(y,X,time,models,num_rand_feat,...
    len_scale_space_w,len_scale_time_w, len_scale_space_w0,len_scale_time_w0, fold)
...
for iter = 1:2000 % should be 20k
    ...
    PhiW = sqrt(2/num_rand_feat)*cos(Z*X(idx,:)'/len_scale_space_w + Zt*time(idx)'/len_scale_time_w + piZ*ones(1,batch_size));
    PhiW0 = sqrt(2/num_rand_feat)*cos(Z*X(idx,:)'/len_scale_space_w0 + Zt*time(idx)'/len_scale_time_w0 + piZ*ones(1,batch_size));
    ...
    grad = PhiW*((1/noise)*repmat(error,num_models,1).*(models(idx,:)' - repmat(model_avg,num_models,1)).*softmax)' - lambda*W;
    ...
    w0tmp = inv(lambda0*noise*eye(num_rand_feat) + PhiW0*PhiW0')*(PhiW0*residual);
    % 2d.viii not sure 
...
for iter = 1:floor(num_obs/batch_size)-1
    PhiW = sqrt(2/num_rand_feat)*cos(Z*X(iter*batch_size+1:iter*batch_size+batch_size,:)'/len_scale_space_w + Zt*time(iter*batch_size+1:iter*batch_size+batch_size)'/len_scale_time_w + piZ*ones(1,batch_size));
    PhiW0 = sqrt(2/num_rand_feat)*cos(Z*X(iter*batch_size+1:iter*batch_size+batch_size,:)'/len_scale_space_w0 + Zt*time(iter*batch_size+1:iter*batch_size+batch_size)'/len_scale_time_w0 + piZ*ones(1,batch_size));
    dotWPhi = W'*PhiW;
    softmax = exp(dotWPhi);
    softmax = softmax./repmat(sum(softmax,1),num_models,1);
        ...
        for j = i:num_models
            if i == j
                SigW((i-1)*num_rand_feat+1:(i-1)*num_rand_feat+num_rand_feat,(i-1)*num_rand_feat+1:(i-1)*num_rand_feat+num_rand_feat) = ...
                    SigW((i-1)*num_rand_feat+1:(i-1)*num_rand_feat+num_rand_feat,(i-1)*num_rand_feat+1:(i-1)*num_rand_feat+num_rand_feat) ...
                        -lambda*eye(num_rand_feat)/floor(num_obs/batch_size) + (1/noise)*((repmat(t1(i,:) + t2(i,:),num_rand_feat,1).*PhiW)*PhiW');
            elseif bool_global_cov
                ...
                SigW((i-1)*num_rand_feat+1:(i-1)*num_rand_feat+num_rand_feat,(j-1)*num_rand_feat+1:(j-1)*num_rand_feat+num_rand_feat) = ...
                    SigW((i-1)*num_rand_feat+1:(i-1)*num_rand_feat+num_rand_feat,(j-1)*num_rand_feat+1:(j-1)*num_rand_feat+num_rand_feat) + ...
                    (repmat((1/noise)*(tij1+tij2+tij3),num_rand_feat,1).*PhiW)*PhiW';
    ...
    % I think this one is for the bias. 
    SigW(num_models*num_rand_feat+1:num_models*num_rand_feat+num_rand_feat,num_models*num_rand_feat+1:num_models*num_rand_feat+num_rand_feat) = ...
        SigW(num_models*num_rand_feat+1:num_models*num_rand_feat+num_rand_feat,num_models*num_rand_feat+1:num_models*num_rand_feat+num_rand_feat) ...
        -lambda*eye(num_rand_feat)/floor(num_obs/batch_size) - (1/noise)*PhiW0*PhiW0';
    if bool_global_cov
        for k = 1:num_models
           SigW(num_models*num_rand_feat+1:num_models*num_rand_feat+num_rand_feat,(k-1)*num_rand_feat+1:(k-1)*num_rand_feat+num_rand_feat) = ...
               SigW(num_models*num_rand_feat+1:num_models*num_rand_feat+num_rand_feat,(k-1)*num_rand_feat+1:(k-1)*num_rand_feat+num_rand_feat) ...
               -(1/noise)*(repmat((models(iter*batch_size+1:iter*batch_size+batch_size,k)'-model_avg).*softmax(k,:),num_rand_feat,1).*PhiW0)*PhiW0';
