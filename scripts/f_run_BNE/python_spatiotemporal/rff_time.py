...
class RFF_TIME:
	...
	def __init__(self, X_train, D=100, h=1.0, ht=1.0):
		"""
		...
		self.GAMMA = np.random.normal(size=(self.D, self.d))
		self.GAMMA[:, 0:(self.d-1)] *= (1 / np.sqrt(h))
		self.GAMMA[:, self.d - 1] *= (1 / np.sqrt(ht))
	...
	def get_Z(self, X):
		...
		# Input
		X : An m x d matrix containing "raw" data points.
