...
save_folds <- function(foldNum, YYYY){
  ...
  train.fold %>% 
    ...
    dplyr::select(lat, lon, obs_pm2_5, av_pred, gs_pred, cmaq_outs_pred, js_pred, caces_pred, ref_id, region) %>%
  ...
  testFold <- train.fold %>% 
    ...
    dplyr::select(lat, lon, av_pred, gs_pred, cmaq_outs_pred, js_pred, caces_pred, obs_pm2_5, ref_id, region) %>%
    dplyr::mutate(region = as.numeric(stringr::str_remove(region, 'Region'))) %>%
