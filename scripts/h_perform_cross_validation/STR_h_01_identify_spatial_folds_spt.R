...
# 4a Create hierarchial clusters (not the folds) 
...
save_folds <- function(foldNum, YYYY){
  #foldNum <- 1; YYYY <- 'combined'
  
  # 5b.ii Make the name of the fold
  activeFold <- paste0('fold', str_pad(foldNum, 2, 'left', '0'))
  
  if (YYYY == 'combined') {
    activeYYYY <- 2010:2015
    dir.data <- 'combined_annual'; descrip <- activeFold
    } else if (YYYY != 'combined') {
      activeYYYY <- as.numeric(YYYY)
    dir.data <- 'individual_annual'; descrip <- paste0(activeYYYY, '_', activeFold)
    }
  ...
  train.fold %>% 
    filter(year %in% activeYYYY) %>%
    dplyr::filter(role == 'Train') %>% 
    dplyr::select(lat, lon, year, obs_pm2_5, av_pred, gs_pred, cmaq_outs_pred, js_pred, caces_pred, merra_pred,ref_id, region) %>%
    dplyr::mutate(region = as.numeric(stringr::str_remove(region, 'Region'))) %>%
    readr::write_csv(here::here('BNE_inputs', 'training_datasets', dir.data, 
                         paste0('training_avgscmjsccme_',  descrip, '.csv')))
 
   # 5b.iv save the testing dataset
  train.fold <- train.fold %>% 
    filter(year %in% activeYYYY) %>%
    dplyr::filter(role == 'Test') %>% 
    dplyr::select(lat, lon, year, av_pred, gs_pred, cmaq_outs_pred, js_pred, caces_pred,merra_pred, obs_pm2_5, ref_id, region) %>%
    dplyr::mutate(region = as.numeric(stringr::str_remove(region, 'Region'))) %>%
    readr::write_csv(here::here('BNE_inputs', 'prediction_datasets', dir.data,
                         paste0('predictions_avgscmjsccme_', descrip, '.csv')))
  
  # save the count (only used for spatial bne)
  if (YYYY != 'combined') {
    data.frame(Count = nrow(train.fold)) %>%
    readr::write_csv(here::here('BNE_inputs', 'prediction_datasets', 'individual_annual', 
                                paste0('predCount_avgscmjsccme_', descrip, '.csv')))
  }
}

# now actually make the folds
purrr::map2(rep(1:10, 7), sort(rep(c('combined',2010:2015), 10)), save_folds)
