...
training.full <- foreach(
  ...
  training.year.sf <- training.year %>%
    sf::st_as_sf(coords = c("lon", "lat"), crs=sf::st_crs("epsg:4326"))
  training.year$me_index <- nngeo::st_nn(training.year.sf, me.sf, k = 1)%>% unlist
  
  # 3F.f.ii attach the correct predictions 
  training.year <- me %>% 
    slice(training.year$me_index) %>% 
  ...
  # remember the key is already in order wrt the prediction dataset 
  # because of the arranging we did earlier. 
  preds.year.sf <- preds.year %>%
    sf::st_as_sf(coords = c("ref_lon", "ref_lat"), crs=sf::st_crs("epsg:4326"))
  preds.year$me_index <- nngeo::st_nn(preds.year.sf, me.sf, k = 1)%>% unlist
  
  preds.year <- me %>% 
    slice(preds.year$me_index) %>% 
    dplyr::select(pred_me) %>%
    bind_cols(preds.year)
  ...
  preds.year %>% 
    ...
    readr::write_csv(here::here('inputs', 'pm25', 'prediction_datasets', 
                         'annual_individual', 
                         paste0('preds_annual_', yyyy, '_meBlend.csv')))
