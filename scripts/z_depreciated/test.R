...
for (i in 1:length(YYYYlist)){
  ...
  # this aqs dataset is restricted to conus
  training <- read_csv(here::here('inputs', 'pm25', 'ground_truth', 'formatted', 
                                  'lgc_annual_data_2000-2016_conus.csv'))
  
                                      
  # 2A.a. filter to the right year
  training <-  training %>% 
  ...
  paths <- c(
    # JS 
    here::here('inputs', 'pm25', 'base_models', 'annual', 'js', 
               paste0(YYYYlist[i], '.rds')),
    # AV
    here::here('inputs', 'pm25', 'base_models', 'annual', 'av', 
               paste0('V4NA03_PM25_NA_', YYYYlist[i], '01_', YYYYlist[i], '12-RH35.nc')),
    # GS 
    here::here('inputs', 'pm25', 'base_models', 'annual','gs', 
                'GBD2016_PREDPOP_FINAL.RData'), 
    #CMout
    here::here('inputs', 'pm25', 'base_models', 'annual', 'cmaq_outs',
               paste0('cmout_annual_',YYYYlist[i], '_formatted.fst')),
    # CACES
    here::here('inputs', 'pm25', 'base_models', 'annual', 'cmaq_ins', 
               paste0('CACES_annual_', YYYYlist[i], '_blockGrp_raw.csv')),
    # MERRA
    here::here('inputs', 'pm25', 'base_models', 'annual', 'merra', 
  ...
  for (j in 1:length(datasetCodes)){
    ...
    training$obs_pm2_5 = 2
    training <- training %>% mutate(ref_id = row_number())
    # 2B.d. conduct spatial join 
    training <- spatioTemporalJoin(refData = training,
                                       modelData = modelData,
  ...
  training %>% 
    ...
    dplyr::select(lat, lon, time, obs, av_pred, gs_pred, cmaq_outs_pred, js_pred, caces_pred, merra_pred, ref_id) %>%
    readr::write_csv(here::here('inputs', 'pm25', 'training_datasets', 'annual_individual', 
                                paste0('training_main_', YYYYlist[i], '.csv')))
