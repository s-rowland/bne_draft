...
param.table <- expand.grid(len_scale_sp = c(1.5), 
                           len_scale_t = 'spatialOnly')
...
CV.metrics <- pmap(list(
  list(c('av', 'gs', 'cm', 'js', 'cc'), c('av', 'gs', 'cm', 'js', 'cc', 'me')), 
  1.5, 
  'spatialOnly', 
  'all', 
  'all',
...
CV.metrics %>% 
  ...
  arrange(covErr) %>%
  arrange(desc(Rsq)) %>%
  readr::write_csv(here::here(dir.proj, 'outputs_merra', 'b_fit_bne', 
                              'cvmetrics_annual_BNE_sp.csv'))
